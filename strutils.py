...
def write_log(email_found: bool, location_found: bool, location: str, url_found: bool, url: str, form_sent: bool) -> None:
    '''Takes a three value status report and writes it to a logfile in the current directory. Logfile is named by date at YYYY-MM-DD, log entry is timed at HH:MM:SS.
    ...
    - `location`: str, describes the actual location of the property.
    - `url_found`: boolean, describes if a url was found matching the location name on the agency's website.
    - `url`: str, describes the url of the property on the agency's website.
